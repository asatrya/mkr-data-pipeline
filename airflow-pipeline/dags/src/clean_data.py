...
def clean_data(**kwargs):
    """
    Download CSV file from data lake, clean it, 
    and store it again on data lake bucket in CSV format
    """
    ...
    try:
        ...
        if dataLakeMinioClient.bucket_exists(DATALAKE_MINIO_RAW_BUCKET):
            logging.info(
                "[DATA-LAKE] Bucket `{}` is exist".format(DATALAKE_MINIO_RAW_BUCKET))
        else:
        ...
        data = dataLakeMinioClient.get_object(
            DATALAKE_MINIO_RAW_BUCKET, source_object_name)
        with open(destination, 'wb') as file_data:
            for d in data.stream(32*1024):
                file_data.write(d)
            logging.info("[DATA-LAKE] Successfully downloaded file from `{}/{}` to `{}`".format(
                DATALAKE_MINIO_RAW_BUCKET, source_object_name, destination))

        # add to pandas dataframe
        df = pd.read_csv(destination)
        logging.info("Successfully read data from {}".format(destination))

        # transformation: replace "'" chars in Account No column
        df['Account No'] = df['Account No'].str.replace("'", "")

        # transformation: change date format to ISO format (it is assumed that the timezone is already in UTC)
        df['DATE'] = pd.to_datetime(df['DATE'], format='%Y-%m-%d')
        df['DATE'] = df['DATE'].map(
            lambda x: datetime.datetime.strftime(x, '%Y-%m-%dT%H:%M:%SZ'))

        # transformation: change value date format to ISO format (it is assumed that the timezone is already in UTC)
        df['VALUE DATE'] = pd.to_datetime(df['VALUE DATE'], format='%Y-%m-%d')
        df['VALUE DATE'] = df['VALUE DATE'].map(
            lambda x: datetime.datetime.strftime(x, '%Y-%m-%dT%H:%M:%SZ'))
        ...
        if dataLakeMinioClient.bucket_exists(DATALAKE_MINIO_CLEANED_BUCKET):
            logging.info("[DATA-LAKE] Bucket `{}` is exist".format(
                DATALAKE_MINIO_CLEANED_BUCKET))
        else:
            logging.info("[DATA-LAKE] Bucket `{}` is not exist, will attempt to create".format(
                DATALAKE_MINIO_CLEANED_BUCKET))
            dataLakeMinioClient.make_bucket(DATALAKE_MINIO_CLEANED_BUCKET)
            logging.info("[DATA-LAKE] Bucket `{}` is created".format(
                DATALAKE_MINIO_CLEANED_BUCKET))

        dataLakeMinioClient.fput_object(
            DATALAKE_MINIO_CLEANED_BUCKET, source_object_name, destination, content_type='application/csv')
        logging.info(
